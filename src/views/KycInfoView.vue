...
onMounted(async () => {
    ...
    try {
        ...
        if (response.ok && data.code === 0) {
            ...
            // kyc_level === 1 && kyc_step === 1 -> Allow access (fill form)
            else if (user.kyc_level === 1 && user.kyc_step === 1) {
                // Stay here
            }
            // Other cases (e.g. already verified level 2, or weird state) -> Back to account
            else {
                router.push('/account')
            }
        } else {
             // Failed to fetch user info, safe default -> back to account
             router.push('/account')
        }
    } catch (error) {
...
const handleSubmit = async () => {
  ...
  // Double check age on submit
  if (getAge(formData.value.birth_date) < 18) {
      dateError.value = '未滿18歲不可進行驗證。'
      errorMessage.value = '未滿18歲不可進行驗證。'
      ...
      <div class="col-md-8 col-lg-6">
        <div class="card shadow-lg border-0 rounded-3">
          <div class="card-body p-5">
            ...
            <!-- Loading State -->
            <div v-if="isCheckingStatus" class="text-center py-5">
                 <div class="spinner-border text-primary" role="status">
                    <span class="visually-hidden">Loading...</span>
                 </div>
                 <p class="mt-3 text-muted">正在檢查驗證狀態...</p>
            </div>

            <!-- Failure State -->
            <div v-else-if="isVerificationFailed" class="text-center py-5">
                 <i class="bi bi-x-circle-fill text-danger display-1 mb-4"></i>
                 <h3 class="fw-bold text-danger mb-3">KYC驗證失敗請重新驗證</h3>
                 <p class="text-muted mb-4">您的身分驗證未能通過，請聯繫客服或返回會員中心。</p>
                 <button class="btn btn-primary btn-lg px-5" @click="$router.push('/account')">
                     前往會員中心
                 </button>
            </div>
            ...
            <div v-else>
                ...
                <div class="row mb-3">
                    ...
                    <input type="text" class="form-control" v-model="formData.first_name" placeholder="e.g. 大明">
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <div v-if="dateError" class="text-danger small mt-1 fw-bold">
                    {{ dateError }}
                    </div>
                </div>
